...
use crate::config::{AliasMap, config_file, MissingRuntimeBehavior};
...
use crate::toolset::{Toolset, ToolSource, ToolVersionList, ToolVersionOptions, ToolVersionRequest};
...
impl RtxToml {
    ...
    fn parse_tool_version_list(
        ...
        match v {
            ...
            v => match v.as_array() {
                ...
                _ => {
                    tool_version_list.requests.push(self.parse_tool_version(key, v, plugin_name)?);
                }
            },
        }
            ...
            Some(table) => {
                if let Some(v) = table.get("version") {
                    ...
                    match prefix.as_str() {
                        Some(s) => {
                            let s = self.parse_template(key, s)?;
                            tv = ToolVersionRequest::Prefix(plugin_name.clone(), s.into());
                        }
                    ...
                    match r.as_str() {
                        Some(s) => {
                            let s = self.parse_template(key, s)?;
                            tv = ToolVersionRequest::Ref(plugin_name.clone(), s.into());
                        }
    ...
    fn parse_tool_version_request(&mut self, key: &str, v: &Value, plugin_name: &PluginName) -> Result<ToolVersionRequest> {
        match v.as_str() {
            Some(s) => {
                let s = self.parse_template(key, s)?;
    ...
    fn replace_versions(&mut self, plugin_name: &PluginName, versions: &[String]) {
        if let Some(plugin) = self.toolset.versions.get_mut(plugin_name) {
            plugin.requests = versions
                .iter()
                .map(|s| (ToolVersionRequest::new(plugin_name.clone(), s), Default::default()))
                .collect();
        }
        let tools = self
            .doc
