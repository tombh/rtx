...
impl ExternalPluginCache {
    pub fn list_bin_paths(&self, config: &Config, plugin: &ExternalPlugin, tv: &ToolVersion) -> &CacheManager<Vec<PathBuf>> {
        let r = self.list_bin_paths.read().unwrap();
        match r.get(&tv.request) {
            Some(c) => c,
            None => {
                let mut w = self.list_bin_paths.write().unwrap();
                w.entry(tv.request.clone()).or_insert_with(|| {
                    ...
                    CacheManager::new(list_bin_paths_filename)
                        ...
                        .with_fresh_file(tv.install_path.to_path_buf())
                })
            }
        }
    }
