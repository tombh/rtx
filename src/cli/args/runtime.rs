...
impl RuntimeArg {
    pub fn parse(input: &str) -> Self {
        match input.split_once('@') {
            Some((plugin, version)) => {
                Self { plugin: plugin.to_string(), tvr: Some(ToolVersionRequest::new(plugin.to_string(), version)) }
            }
            None => Self {
                plugin: input.into(),
                tvr: None,
    ...
    pub fn with_version(self, version: &str) -> Self {
        Self { tvr: Some(ToolVersionRequest::new(self.plugin.clone(), version)), ..self }
    }
}
...
impl Display for RuntimeArg {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match &self.tvr {
            Some(tvr) => write!(f, "{}@{}", self.plugin, tvr.to_string()),
