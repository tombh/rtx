...
use crate::toolset::{ToolsetBuilder, ToolVersionOptions, ToolVersionRequest};
                        ...
                        match ts.versions.get(&runtime.plugin) {
                            Some(tvl) => {
                                for (tvr, opts) in &tvl.requests {
                                    tool_version_requests.push((runtime.plugin.clone(), tvr.clone(), opts.clone()));
                                }
                            }
                            None => {
                                let tvr = ToolVersionRequest::Version(runtime.plugin.clone() , "latest".into());
                                tool_version_requests.push((runtime.plugin, tvr, default_opts.clone()));
                            }
                        }
                    }
                }
        ...
        ThreadPoolBuilder::new()
            ...
            .install(|| -> Result<()> {
                ...
                for (plugin_name, tvr, opts) in tool_version_requests {
                    ...
                    tool_versions.push((&plugin, tvr.resolve(&config, &plugin, opts, ts.latest_versions)?));
                }
                ...
                if !to_uninstall.is_empty() {
                    to_uninstall
                        ...
                        .map(|(plugin, tv)| {
                            ...
                            match plugin.uninstall_version(&config.settings, &tv, &pr, false) {
                                Ok(_) => {
                                    pr.finish();
                                    Ok(())
                                }
